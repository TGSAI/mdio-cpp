...
class VariableCollection {
  public:
    ...
    VariableCollection(std::initializer_list<std::pair<const std::string, Variable<>>> list) : variables(list) {
    }
     ...
     * found or an error occurs during type casting.
     */
    ...
    Result<Variable<T, R, M>> get(const std::string& label) const {
        if (!variables.count(label)) {
            return absl::NotFoundError("Label '" + label + "' not found in the stores map");
        }

        auto cast_store = tensorstore::StaticCast<tensorstore::TensorStore<T, R, M>>(variables.at(label).get_store());
    ...
    /**
    * Lightweight retrieval of a variable from the dataset based on the given label.
    * Does not provide any type casting. Use get() for type casting.
    *
    * @tparam T The data type of the variable. Defaults to `void`.
    * @tparam R The rank of the variable. Defaults to `mdio::dynamic_rank`.
    * @tparam M The read-write mode of the variable. Defaults to `mdio::ReadWriteMode::dynamic`.
    ...
    */
    ...
    /**
     * @brief Checks if the VariableCollection contains a variable with the specified label.
     ...
     * A helper function to get the keys of all the variables that the VariableCollection contains.
     * Order of the keys is not significant.
     * This is intended for internal use for coordinate retrieval.
     * @return A vector of strings containing the keys of all the variables in the VariableCollection.
     */
     ...
     * @return A sorted vector of the keys of all the variables in the VariableCollection.
    */
    std::vector<std::string> get_iterable_accessor() const {
        // The intention here is to hide the std iterators from the user.
        // unordered_map iterators are not guaranteed to have the same order across different compilers.
  ...
  private:
    ...
    using const_iterator = std::unordered_map<std::string, Variable<>>::const_iterator;
