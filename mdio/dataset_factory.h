 ...
 * This function is intended to be an internal helper function for formatting Variable specs
 ...
 * This function is intended to be an internal helper function for formatting Variable specs
 * It presupposes that all dtypes are little endian or endianless if bool.
 * @param dtype A string representing the dtype of a Variable
 * @return A string representing the dtype in numpy format limited to the dtypes supported by MDIO Dataset
 */
 ...
 * This function is intended to be an internal helper function for formatting Variable specs
 * It will modify with side-effect on "input"
 * @param input A MDIO Variable spec
 * @param variable A Variable stub (Will be modified)
 * @return OkStatus if successful, InvalidArgumentError if dtype is not supported
 */
 ...
 * @param input A MDIO Variable spec
 * @param variable A Variable stub (Will be modified)
 * @return OkStatus if successful, InvalidArgumentError if compressor is invalid for MDIO
 */
absl::Status transform_compressor(nlohmann::json& input, nlohmann::json& variable) {
 ...
 * @return void -- Can only be successful because validation must always pass before this step
 * This presumes that the user does not attempt to use these functions directly
 */
void transform_shape(nlohmann::json& input,
 ...
 */
absl::Status transform_metadata(const std::string& path, nlohmann::json& variable) {
    std::string bucket = "NULL"; // Default value, if is NULL don't add a bucket field
    ...
    if (driver != "file") {
        ...
        // Ensure _path has a trailing slash or the mdio file will not be created properly
        if (_path.back() != '/') {
            _path += '/';
        }
        ...
        if (file_parts.size() < 2) {
            return absl::InvalidArgumentError("Cloud path requires [gs/s3]://[bucket]/[path to file] name");
        }
        bucket = file_parts[0];
...
/**
 * @brief Constructs an MDIO Variable spec from an MDIO Dataset Variable list element
 * This function is intended to be an internal helper function for formatting Variable specs
 * @param json A MDIO Dataset Variable list element
 * @param dimensionMap A map of dimension names to sizes
 * @return A Variable spec or an error if the Variable spec is invalid
 */
tensorstore::Result<nlohmann::json>
from_json_to_spec(nlohmann::json& json, std::unordered_map<std::string, int>& dimensionMap, const std::string& path) {
    ...
    if (json.contains("metadata")) {
        if (json["metadata"].contains("chunkGrid")) {
            variableStub["metadata"]["chunks"] = json["metadata"]["chunkGrid"]["configuration"]["chunkShape"];
        ...
        if (!json["dataType"].contains("fields")) {
            ...
            // We're going to use the variable dtype because it's already in byte format
            std::string dtype;
    ...
    // I think the longName field should be optional for a Variable but this ensures the spec is valid.
 ...
 * This function is intended to be an internal helper function for formatting Variable specs
 * It's intended to be used while the Variables are considered to be the same size.
 * This behavior will need to change as MDIO gains functionality
 * @param spec A Dataset spec
 * @return A map of dimension names to sizes or error if the dimensions are not consistently sized
 */
tensorstore::Result<std::unordered_map<std::string, int>> get_dimensions(nlohmann::json& spec) {
            ...
            for (auto& dimension : variable["dimensions"]) {
                if (dimensions.count(dimension["name"]) == 0) {
                    ...
                    if (dimensions[dimension["name"]] != dimension["size"]) {
                        return absl::InvalidArgumentError("Dimension " + dimension["name"].dump() +
 ...
 * This should be the only function called by the user to construct a Dataset from a spec
 * @param spec A Dataset spec
 * @return A vector of Variable specs or an error if the Dataset spec is invalid
 */
tensorstore::Result<std::tuple<nlohmann::json, std::vector<nlohmann::json>>> Construct(nlohmann::json& spec,
                                                                                       const std::string& path) {
    // Validation should only return status codes. If it returns data then it should be a "constructor"
    auto status = validate_dataset(spec);
    if (!status.ok()) {
        return status;
    }

    // This made more sense to validate in the constructor because I require this data
