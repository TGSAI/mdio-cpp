 ...
 * @return An `mdio::Result` containing the .zarray JSON metadata on success, or an error on failure.
 */
Result<nlohmann::json> get_zarray(const ::nlohmann::json metadata) {
    // derive .zarray json metadata (without reading it).
    auto json = metadata; // Why am I doing this? It's an extra copy that does nothing!
    nlohmann::json zarray;
    if (!json.contains("metadata")) {
        json["metadata"] = nlohmann::json::object();               // Just add an empty object
        json["metadata"]["attributes"] = nlohmann::json::object(); // We need attributes as well
    }
    ...
    if (!json["metadata"].contains("chunks") && json["metadata"].contains("shape")) {
        zarray["chunks"] = json["metadata"]["shape"];
    } else {
        zarray["chunks"] = json["metadata"]["chunks"]; // TODO: Can this have more states?
    }
    ...
    MDIO_ASSIGN_OR_RETURN(auto zarr_metadata, tensorstore::internal_zarr::ZarrMetadata::FromJson(zarray))
 ...
 */
Future<void> write_zmetadata(const ::nlohmann::json& dataset_metadata,
    ...
    std::string driver = json_variables[0]["kvstore"]["driver"].get<std::string>();

    for (const auto& json : json_variables) {
        zarray_key = std::filesystem::path(json["kvstore"]["path"]).stem() / ".zarray";
        zattrs_key = std::filesystem::path(json["kvstore"]["path"]).stem() / ".zattrs";
        ...
        // We do not want to be seralizing the variable_name. It should be self-describing
        fixedJson.erase("variable_name");
        if (fixedJson.contains("long_name") && fixedJson["long_name"].get<std::string>() == "") {
    ...
    std::vector<std::string> file_parts = absl::StrSplit(json_variables[0]["kvstore"]["path"].get<std::string>(), '/');
    size_t toRemove = file_parts.back().size();
    std::string strippedPath = json_variables[0]["kvstore"]["path"].get<std::string>().substr(
        0, json_variables[0]["kvstore"]["path"].get<std::string>().size() - toRemove - 1);
    kvstore["path"] = strippedPath;

    if (driver == "gcs" || driver == "s3") {
        kvstore["bucket"] = json_variables[0]["kvstore"]["bucket"].get<std::string>();
    ...
    auto zattrs_future = tensorstore::MapFutureValue(
        ...
        [zattrs = std::move(zattrs)](const tensorstore::KvStore& kvstore) {
            return tensorstore::kvstore::Write(kvstore, "/.zattrs", absl::Cord(zattrs.dump()));
        },
        kvs_future);
    ...
    auto zmetadata_future = tensorstore::MapFutureValue(
        ...
        [zmetadata = std::move(zmetadata)](const tensorstore::KvStore& kvstore) {
            return tensorstore::kvstore::Write(kvstore, "/.zmetadata", absl::Cord(zmetadata.dump()));
        },
        kvs_future);
    ...
    auto zgroup_future = tensorstore::MapFutureValue(
        ...
        [zgroup = std::move(zgroup)](const tensorstore::KvStore& kvstore) {
            return tensorstore::kvstore::Write(kvstore, "/.zgroup", absl::Cord(zgroup.dump()));
        },
        kvs_future);

    return tensorstore::WaitAllFuture(zattrs_future, zmetadata_future, zgroup_future);
}
 ...
 */
Future<tensorstore::KvStore> dataset_kvs_store(const std::string& dataset_path) {
    ...
    if (file_parts.size() < 2) {
        return absl::InvalidArgumentError("gcs/s3 drivers requires [s3/gs]://[bucket]/[path_to_file]");
    }
 ...
 * It will default to the "file" driver if no prefix is found.
 * @param dataset_path The path to the dataset.
 * @return An `mdio::Future` containing the .zmetadata JSON on success, or an error on failure.
 */
...
from_zmetadata(const std::string& dataset_path) {
    ...
    auto kvs_read_result = tensorstore::kvstore::Read(kvs_future.value(), ".zmetadata").result();

    ::nlohmann::json zmetadata;
    try {
        zmetadata = ::nlohmann::json::parse(std::string(kvs_read_result.value().value));
    ...
    if (!zmetadata.contains("metadata")) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "zmetadata does not contain metadata.");
    }

    if (!zmetadata["metadata"].contains(".zattrs")) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "zmetadata does not contain dataset metadata.");
    }

    auto dataset_metadata = zmetadata["metadata"][".zattrs"];

    std::string driver = "file";
    // TODO: Make this more robust. May be invalid if the stored path gets mangled somehow.
    // Infer the driver
    ...
    if (driver != "file") {
        std::string providedPath = dataset_path.substr(5); // Strip the gs:// or s3://
        size_t bucketLen = providedPath.find_first_of('/');
        bucket = providedPath.substr(0, bucketLen);                                // Extract the bucket name
        cloudPath = providedPath.substr(bucketLen + 1, providedPath.length() - 2); // Extract the path
    }
    ...
    for (auto& element : zmetadata["metadata"].items()) {
        ...
        if (element.key().substr(element.key().find_last_of(".") + 1) == "zarray") {
            std::string variable_name = element.key().substr(0, element.key().find("/"));
            nlohmann::json new_dict = {{"driver", "zarr"},
                                       {"kvstore", {{"driver", driver}, {"path", dataset_path + "/" + variable_name}}}};
            if (driver != "file") {
    ...
    if (!json_vars_from_zmeta.size()) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "Not variables found in zmetadata.");
    }

    return tensorstore::ReadyFuture<std::tuple<::nlohmann::json, std::vector<::nlohmann::json>>>(
        std::make_tuple(dataset_metadata, json_vars_from_zmeta));
}
} // namespace internal

using coordinate_map = std::unordered_map<std::string, std::vector<std::string>>;
...
public:
    ...
    Dataset(const nlohmann::json& metadata,
            const VariableCollection& variables,
            const coordinate_map& coordinates,
            const tensorstore::IndexDomain<>& domain)
        : metadata(metadata), variables(variables), coordinates(coordinates), domain(domain) {}
    ...
    friend std::ostream& operator<<(std::ostream& os, const Dataset& dataset) {
        ...
        for (const auto& key : keys) {
            os << "Variable: " << key << " - Dimensions: " << dataset.variables.at(key).value().dimensions() << "\n";
        }
     ...
     * @return An `mdio::Result` containing the retrieved variable if successful, or an error if the label is not
     * found.
     */
     ...
     * @endcode
     *
     * @return An `mdio::Future` resolves to a Dataset if successful, or an error if
     * the schema is invalid.
     */
    template <typename... Option>
    static Future<Dataset>
    from_json(::nlohmann::json& json_schema, const std::string& path, Option&&... options) {
        ...
        return mdio::Dataset::Open(dataset_metadata, json_vars, std::forward<Option>(options)...);
    }
     ...
     * @endcode
     *
     * @return An `mdio::Result` containing a sliced Dataset if successful, or an error if the slice is invalid.
     */
    ...
    Result<Dataset> isel(Descriptors&... descriptors) {
        ...
        for (const auto& name : keys) {
            MDIO_ASSIGN_OR_RETURN(auto variable,
                                         variables.at(name).value().slice(std::forward<Descriptors>(descriptors)...))
            // add to variable
        ...
        MDIO_ASSIGN_OR_RETURN(
            auto new_domain,
            tensorstore::IndexDomainBuilder<>(size).origin(origin).shape(shape).labels(labels).Finalize())
     ...
     * This method will assume that the Dataset already exists at the specified path.
     * @param dataset_path The path to the dataset.
     * @details \b Usage
     * @code
     ...
     * );
     * @endcode
     * @return An `mdio::Future` containing a Dataset if successful, or an error if the path is invalid.
     */
    template <typename S = std::string, typename... Option>
    static std::enable_if_t<(std::is_same_v<S, std::string>), Future<Dataset>> Open(const S& dataset_path,
                                                                                                 Option&&... options) {

        TENSORSTORE_INTERNAL_ASSIGN_OPTIONS_OR_RETURN(TransactionalOpenOptions, transact_options, options)

        if (transact_options.open_mode != constants::kOpen) {
            return absl::Status(absl::StatusCode::kInvalidArgument, "Open from path is only valid in open-mode.");
        }

        MDIO_ASSIGN_OR_RETURN(auto params_from_zmetadata, mdio::internal::from_zmetadata(dataset_path).result())
     ...
     * This method should be used in conjunction with the dataset_factory Construct function.
     * It expects a fully valid metadata and vector of MDIO compliant Variable specs.
     * @param metadata The metadata for the dataset.
     * @param json_variables A vector of correctly constructed Variable specs.
     * @return An `mdio::Result` containing a Dataset if successful, or an error if the schema is invalid.
     */
    template <typename... Option>
    static Future<Dataset>
    Open(const ::nlohmann::json& metadata, const std::vector<::nlohmann::json>& json_variables, Option&&... options) {
        // I need to know if we are intending to create a dataset from scratch?
        TENSORSTORE_INTERNAL_ASSIGN_OPTIONS_OR_RETURN(TransactionalOpenOptions, transact_options, options)
        bool do_create = transact_options.open_mode == constants::kCreateClean || transact_options.open_mode == constants::kCreate;
        ...
        for (const auto& json : json_variables) {
            ...
            var.ExecuteWhenReady(
                [promise = std::move(pair.promise)](tensorstore::ReadyFuture<mdio::Variable<>> readyVar) {
        ...
        if (do_create) {
            futures.push_back(mdio::internal::write_zmetadata(metadata, json_variables));
        }
        ...
        all_done_future.ExecuteWhenReady([promise = std::move(pair.promise),
            ...
            for (const auto& fvar : variables) {
                ...
                if (meta.contains("coordinates")) {
                    // Because of how Variable is set up, we need to break down a space delimited string to the vector
            ...
            auto dataset_domain =
                tensorstore::IndexDomainBuilder<>(shape_size.size()).shape(values).labels(keys).Finalize();
     ...
     * The new Variable will replace the original one in the Dataset once the future has been resolved.
     * Attempting to access the Variable before the future has been resolved may result in a race condition.
     * @param variableName The name of the variable to select the field from.
     * @param fieldName The name of the field to select.
     * @return An `mdio::Future` if the selection was valid and successful, or an error if the selection was
     * invalid.
     */
    Future<Variable<>> SelectField(const std::string variableName, const std::string fieldName) {
        ...
        if (found == -1) {
            return absl::Status(absl::StatusCode::kInvalidArgument,
                                "Field: '" + fieldName + "' not found in Variable '" + variableName + "'.");
        }
        ...
        if (specJson["kvstore"].contains("bucket")) {
            ...
            cloudPath.pop_back(); // We need to remove the trailing / from a cloud path
        ...
        fieldedVar.ExecuteWhenReady(
            [this, promise = pair.promise, variableName](tensorstore::ReadyFuture<mdio::Variable<>> readyFut) {
    ...
    tensorstore::Future<void> CommitMetadata() {
        ...
        if (modifiedVariables.empty()) {
            tensorstore::Future<void> err = tensorstore::MakeResult<tensorstore::Future<void>>(absl::InvalidArgumentError("No variables were modified."));
            return err;
        }

        // We need to update the entire .zmetadata file
        std::vector<nlohmann::json> json_vars;
        std::vector<Variable<>> vars; // Keeps the Variables in memory. Fix for premature reference decrement in LLVM compiler.
        for (auto key : keys) {
            auto var = variables.at(key).value();
            vars.push_back(var);
            // Get the JSON, drop transform, and add attributes
            nlohmann::json json = var.get_store().spec().value().ToJson(IncludeDefaults{}).value();
            json.erase("transform");
            json.erase("dtype");
            json["metadata"].erase("filters");
            json["metadata"].erase("order");
            json["metadata"].erase("zarr_format");
            // On local file systems there is a trailing slash that needs to be removed.
        ...
        auto zmetadata_future = mdio::internal::write_zmetadata(metadata, json_vars);
        // Finally we can loop through the updated Variables and update them.

        std::vector<tensorstore::Future<tensorstore::TimestampedStorageGeneration>> variableFutures;
        std::vector<tensorstore::Promise<tensorstore::TimestampedStorageGeneration>> promises;
        std::vector<tensorstore::AnyFuture> futures;
        ...
        for (auto key : modifiedVariables) {
            ...
            updateFuture.ExecuteWhenReady([promise = std::move(pair.promise)](tensorstore::ReadyFuture<tensorstore::TimestampedStorageGeneration> readyFut) {
                promise.SetResult(readyFut.result());
            });
        ...
        auto pair = tensorstore::PromiseFuturePair<void>::Make();
        all_done_future.ExecuteWhenReady([promise = std::move(pair.promise), updates = std::move(variableFutures)](tensorstore::ReadyFuture<void> readyFut) {
