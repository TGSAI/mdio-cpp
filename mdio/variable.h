 ...
 * @details \b Usage
 * This provides an example of describing a slice for [0, 100) with a step of 1 along the inline dimension and [0, 200)
 * with a step of 1 along the crossline dimension. The depth dimension would remain wholely intact as it is not
 * specified.
 * @code
...
namespace internal {
/**
 * @brief Validates and processes a JSON specification for a tensorstore variable.
 *
 * This function validates and processes the JSON specification for a zarr V2 tensorstore variable.
 * It expects the JSON specification will have a field called attributes that contains a field called dimension_names.
 * This is a specification of MDIO and is not specifically required by zarr V2
 *
 * @tparam Mode The read-write mode to use.
 * @param json_spec The JSON specification to validate and process.
 * @return A tuple containing the processed JSON specification and a new JSON object with the kvstore and updated
 * variable name.
 * @throws absl::InvalidArgumentError if the JSON specification is invalid (according to the MDIO standard).
 */
template <ReadWriteMode Mode = ReadWriteMode::dynamic>
Result<std::tuple<nlohmann::json, nlohmann::json>>
ValidateAndProcessJson(const nlohmann::json& json_spec) {
    // Check if attributes are in the original JSON
    if (!json_spec.contains("attributes")) {
        return absl::InvalidArgumentError("The json_spec does not contain 'attributes'.");
    }

    // Check if dimensions field exists in attributes
    if (!json_spec["attributes"].contains("dimension_names")) {
        return absl::InvalidArgumentError("The 'attributes' does not contain 'dimension_names'.");
    }
    ...
    new_json["variable_name"] = std::filesystem::path(json_spec["kvstore"]["path"]).stem().string();
 ...
 *
 * Parses the JSON spec for attributes constructs the Variable object according to the MDIO standard.
 *
 * @tparam T The data type of the Variable.
 * @tparam R The rank of the Variable.
 * @tparam M The read-write mode of the Variable.
 * @param spec The JSON specification to parse. `spec` is expected to contain ["attributes"]["variable_name"] and
...
Result<Variable<T, R, M>> from_json(const ::nlohmann::json spec,
                                                 const tensorstore::TensorStore<T, R, M> store) {
    auto attributes = spec;
    // There seems to be a case where we don't actually strip out the super attributes field by this point.
    if (attributes.contains("attributes") && attributes["attributes"].contains("variable_name")) {
    ...
    std::string long_name = ""; // Default case. TODO: Look into making this optional.
    if (scrubbed_spec.contains("long_name")) {
        if(scrubbed_spec["long_name"].is_string() && scrubbed_spec["long_name"].get<std::string>().size() > 0) {
            long_name = scrubbed_spec["long_name"].get<std::string>();
        } else {
            scrubbed_spec.erase("long_name");
        }
    } // Not having this field is fine. If it doesn't get seralized returning an error status is not necessary.

    // These live in our `UserAttributes` object and are technically mutable. The best kind of mutable!
    if (scrubbed_spec.contains("metadata")) {
        scrubbed_spec["metadata"].erase("attributes");
        scrubbed_spec["metadata"].erase("statsV1");
    }

    std::shared_ptr<std::shared_ptr<UserAttributes>> attrs = std::make_shared<std::shared_ptr<UserAttributes>>(std::make_shared<UserAttributes>(attrsRes.value()));
...
/**
 * @brief Creates a new variable with the given attributes and returns a future that will be fulfilled with the created
 * variable. Provide an Open method for a new file ...
 * @tparam T The element type of the variable.
 * @tparam R The rank of the variable.
 * @tparam M The read-write mode of the variable.
 * @param json_store The JSON object representing the TensorStore to create the variable in.
 * @param json_var The JSON object representing the attributes of the variable to create.
 * @param options The transactional open options to use when opening the TensorStore.
 * @throws absl::StatusCode::kInvalidArgument if the attributes are empty.
 * @return mdio::Future<Variable<T, R, M>> A future that will be fulfilled with the created variable.
 */
template <typename T = void,
          DimensionIndex R = dynamic_rank,
          ReadWriteMode M = ReadWriteMode::dynamic,
...
CreateVariable(const nlohmann::json& json_spec, const nlohmann::json& json_var, Option&&... options) {
    auto spec = tensorstore::MakeReadyFuture<::nlohmann::json>(json_var);

    // FIXME - add schematized validation of the variable.
    if (json_var.empty()) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "Expected attributes to be non-empty.");
    }

    if (!json_spec.contains("metadata")) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "Variable spec requires metadata");
    }

    if (!json_spec["metadata"].contains("dtype")) {
        return absl::Status(absl::StatusCode::kInvalidArgument, "Variable metadata requires dtype");
    }

    MDIO_ASSIGN_OR_RETURN(auto zarr_dtype,
                                 tensorstore::internal_zarr::ParseDType(json_spec["metadata"]["dtype"]))

    // Handles the use case of creating a struct array, but intending to open as void.
    auto do_handle_structarray = zarr_dtype.has_fields && !json_spec.contains("field");
    ...
    auto future_json_store = tensorstore::MakeReadyFuture<::nlohmann::json>(json_spec_without_metadata);

    auto publish =
        [](const ::nlohmann::json& json_var, bool isCloudStore, const tensorstore::TensorStore<T, R, M>& store)
        ...
        if (output_json.contains("coordinates")) {
            ...
            if (coords.empty() || (coords.is_string() && coords.get<std::string>() == "")) {
        ...
        // It's important to use the store's kvstore or else we get a race condition on "mkdir".
        return tensorstore::kvstore::Write(store.kvstore(), outpath, absl::Cord(output_json.dump()));
    };
    ...
    auto build = [](const ::nlohmann::json& metadata,
                    const tensorstore::TensorStore<T, R, M>& store) -> Future<Variable<T, R, M>> {
        auto labeled_store = store;

        if (metadata.contains("dimension_names")) {
            auto dimension_names = metadata["dimension_names"].get<std::vector<std::string>>();
            for (DimensionIndex i = 0; i < dimension_names.size(); ++i) {
                MDIO_ASSIGN_OR_RETURN(labeled_store,
    ...
    auto future_store = tensorstore::Open<T, R, M>(json_spec_with_field, std::forward<Option>(options)...);

    auto handled_store = future_store;
    if (do_handle_structarray) {
        handled_store = tensorstore::MapFutureValue(
            tensorstore::InlineExecutor{}, apply_reopen, future_store, spec, future_json_store);
    } else {
        handled_store = std::move(future_store);
    }

    auto variable_future = tensorstore::MapFutureValue(tensorstore::InlineExecutor{}, build, spec, handled_store);
    ...
    auto write_metadata_future =
        tensorstore::MapFutureValue(tensorstore::InlineExecutor{}, publish, spec, isCloudStoreRF, handled_store);
    
    // wait for the variable to be created and the zattrs to be written.
    auto all_done_future = tensorstore::WaitAllFuture(variable_future, write_metadata_future);

    auto pair = tensorstore::PromiseFuturePair<Variable<T, R, M>>::Make();
    all_done_future.ExecuteWhenReady([promise = pair.promise, variable_future = std::move(variable_future)](
 ...
 * @return mdio::Future<Variable<T, R, M>> A future that resolves to the opened Variable.
 */
template <typename T = void,
          DimensionIndex R = dynamic_rank,
          ReadWriteMode M = ReadWriteMode::dynamic,
          typename... Option>
Future<Variable<T, R, M>> OpenVariable(const nlohmann::json& json_store, Option&&... options) {
    ...
    // open a store:
    auto future_store = tensorstore::Open<T, R, M>(store_spec, std::forward<Option>(options)...);
    ...
    auto read = [](const tensorstore::KvStore& kvstore) -> Future<tensorstore::kvstore::ReadResult> {
        return tensorstore::kvstore::Read(kvstore, "/.zattrs");
    };

    // go read the attributes return json ...
    auto parse = [](const tensorstore::kvstore::ReadResult& kvs_read, const ::nlohmann::json& spec) {
    ...
    auto make_variable =
        [variable_name](const ::nlohmann::json& metadata,
                        const tensorstore::TensorStore<T, R, M>& store,
                        const ::nlohmann::json& suppliedAttributes) -> Future<Variable<T, R, M>> {
        ...
        if (new_metadata["attributes"].contains("_ARRAY_DIMENSIONS")) {
            ...
            new_metadata["attributes"]["dimension_names"] = new_metadata["attributes"]["_ARRAY_DIMENSIONS"];
            new_metadata["attributes"].erase("_ARRAY_DIMENSIONS");
        }

        if (new_metadata["attributes"].contains("dimension_names")) {
            auto dimension_names = new_metadata["attributes"]["dimension_names"].get<std::vector<std::string>>();
            for (DimensionIndex i = 0; i < dimension_names.size(); ++i) {
                MDIO_ASSIGN_OR_RETURN(labeled_store,
                                             labeled_store | tensorstore::Dims(i).Label(dimension_names[i]));
            }
        } else {
            return absl::NotFoundError(absl::StrCat("Field not found in JSON: ", "metadata"));
        }

        if (!suppliedAttributes.is_null()) {
            // The supplied attributes contain some things that we do not serialize. We need to remove them.
            // This could cause confusion. If the user specifies a different chunkGrid, it will not be used and should actually fail here.
            // TODO: Figure out if we can acquire the chunking information. I don't believe we can without making this serial.
            ...
            if (correctedSuppliedAttrs["attributes"].contains("metadata")) {
                ...
                for (auto& item : correctedSuppliedAttrs["attributes"]["metadata"].items()) {
                    correctedSuppliedAttrs["attributes"][item.key()] = std::move(item.value());
                }
            ...
            searchableMetadata["attributes"].erase(
                "variable_name"); // Since we don't actually want to have to specify the variable name
            if (searchableMetadata["attributes"].contains("metadata")) {
                for (auto& item : searchableMetadata["attributes"]["metadata"].items()) {
                    searchableMetadata["attributes"][item.key()] = std::move(item.value());
                }
            ...
            while (!queue.empty()) {
                ...
                for (auto& [key, value] : currentMetadata.items()) {
                    if (!currentAttributes.contains(key)) {
                        return absl::NotFoundError(absl::StrCat("Field not found in JSON: ", key));
                    }
                    if (value.is_object() && currentAttributes[key].is_object()) {
                        // If the value is a JSON object, add it to the queue for further processing
                        queue.push({value, currentAttributes[key]});
                    } else if (value != currentAttributes[key]) {
                        return absl::InvalidArgumentError(absl::StrCat("Conflicting values for field: ",
                                                                       key,
                                                                       ". ",
                                                                       "Expected: ",
                                                                       value.dump(),
                                                                       ", but got: ",
                                                                       currentAttributes[key].dump()));
                    }
                }
            }
        }
    ...
    auto kvs_read_future = tensorstore::MapFutureValue(tensorstore::InlineExecutor{}, read, kvs_future);

    auto metadata = tensorstore::MapFutureValue(tensorstore::InlineExecutor{}, parse, kvs_read_future, spec);

    return tensorstore::MapFutureValue(tensorstore::InlineExecutor{},
                                       make_variable,
                                       metadata,
                                       future_store,
...
Future<Variable<T, R, M>> Open(const nlohmann::json& json_spec,
    ...
    if (options.open_mode == constants::kCreateClean || options.open_mode == constants::kCreate) {
...
public:
    ...
    Variable(const std::string& variableName,
        ...
        : variableName(variableName), longName(longName), metadata(metdata), store(store), attributes(attributes) {
            attributesAddress = reinterpret_cast<std::uintptr_t>((*attributes).get());
        };

    // Allows for conversion to compatible types (SourceElement), which should always be possible to void.
    template <typename SourceElement, DimensionIndex SourceRank, ReadWriteMode SourceMode>
    Variable(const Variable<SourceElement, SourceRank, SourceMode>& other)
        : variableName(other.get_variable_name()), longName(other.get_long_name()), metadata(other.getReducedMetadata()), store(other.get_store()), attributes(other.attributes), attributesAddress(attributesAddress){};
     ...
     * @tparam M The read/write mode of the variable. Defaults to `mdio::ReadWriteMode::dynamic`.
    ...
    static std::enable_if_t<(tensorstore::IsCompatibleOptionSequence<TransactionalOpenOptions, Option...>),
                            Future<Variable<T, R, M>>>
    Open(const nlohmann::json& json_spec, Option&&... option) {
        TENSORSTORE_INTERNAL_ASSIGN_OPTIONS_OR_RETURN(TransactionalOpenOptions, options, option)
     ...
     * @return A future of VariableData that will be ready when the read is complete.
     */
    ...
    Future<VariableData<T, R, OriginKind>> Read() {
        ...
        VariableData<T, R, OriginKind> variableData{variableName, longName, getMetadata(), labeledArray};

        return tensorstore::MakeReadyFuture<VariableData<T, R, OriginKind>>(variableData);
    }

    /**
     * @brief Write the data to the variable.
     * Writes the data from the source variable data to the target variable.
     * @param source A VariableData object with the data to write. This is the in-memory representation of the data.
     * @param target A Variable object with the target store. This is the non-volitile representation of the data.
     *
     * @details \b Usage
     * This provides an example of writing the data from the source variable data to the target variable.
     * @code
     * MDIO_ASSIGN_OR_RETURN(auto velocity, mdio::Variable<>::Open(velocity_path, mdio::constants::kOpen));
    ...
    WriteFutures Write(const VariableData<T, R, OriginKind> source) const {
        if (source.dtype() != this->dtype()) {
            return absl::InvalidArgumentError("The source and target dtypes do not match.");
        }
     ...
     */
    IndexDomainView<R> dimensions() const {
        return store.domain();
    }
     ...
     * of the variable.
     * @return An `mdio::Result` object containing the specification of the variable.
     */
    Result<Spec> spec() const {
        return store.spec();
    }
     ...
     * @return A slice descriptor that will not go out-of-bounds for the given Variable.
     */
        ...
        for (size_t idx = 0; idx < labels.size(); ++idx) {
            if (labels[idx] == desc.label) {
                ...
                return {desc.label,                                                             // label
                        desc.start < domain.origin()[idx] ? domain.origin()[idx] : desc.start,  // start
                        desc.stop > domain.shape()[idx] ? domain.shape()[idx] : desc.stop,      // stop
                        desc.step};                                                             // step
            }
        }
        // We don't slice along a dimension that doesn't exist, so the descriptor is valid
        return desc;
    }

    /**
     * @brief Slices the Variable along the specified dimensions and returns the resulting sub-Variable.
     * This slice is performed as a half open interval.
     ...
     * @post The resulting Variable will be sliced along the specified dimensions within it's domain. 
     * If the slice lay outside of the domain of the Variable, the slice will be clamped to the domain.
     * @param descriptors The descriptors used to specify the slice.
     * @details \b Usage
     * This provides an example of slicing the Variable along the inline and crossline dimensions.
     * @code
     * mdio::SliceDescriptor desc1 = {"inline", 0, 100, 1};
     * mdio::SliceDescriptor desc2 = {"crossline", 0, 200, 1};
    ...
    Result<Variable> slice(const Descriptors&... descriptors) const {
                ...
                ((
                     [&] {
                        ...
                        if (clampedDesc.start > clampedDesc.stop) {
                         ...
                         return 0; // Return a dummy value to satisfy the comma operator
                     }(),
                     idx++),
                 ...);
            },
        ...
        if (preconditionStatus == -2) {
            return absl::InvalidArgumentError("Only step 1 is supported for slicing.");
            ...
            std::apply(
                ...
                tuple_descs);
            return absl::InvalidArgumentError(std::string("Slice descriptor for ") + std::string(err.label.label()) + 
                                  " had an illegal configuration.\n\tStart '" + std::to_string(err.start) + 
                                  "' greater than or equal to stop '" + std::to_string(err.stop) + "'.");
        }

        if (labels.size()) {
            MDIO_ASSIGN_OR_RETURN(auto slice_store,
                                         store | tensorstore::Dims(labels).HalfOpenInterval(start, stop, step));
            // return a new variable with the sliced store
            return Variable{variableName, longName, metadata, slice_store, attributes};
        } else {
     ...
     *  // Build descriptors to slice out a swath, here we will get every chunk in the z-direction
     *  mdio::SliceDescriptor desc1 = {"inline", chunkShape[0], chunkShape[0] * 2, 1};
     *  mdio::SliceDescriptor desc2 = {"crossline", chunkShape[1], chunkShape[1] * 2, 1};
     * @endcode
     * @return An NotFoundError if the chunk shape could not be retrieved, otherwise a vector of the chunk shape.
     */
    Result<std::vector<DimensionIndex>> get_chunk_shape() const {
        // TODO: Depricate this method name to reduce confusion between it and get_store_shape
        ...
        if (!json["metadata"].contains("chunks")) {
            return absl::NotFoundError("Metadata['attributes'] did not contain key 'chunks'.");
        }
     ...
     * @return A NotFoundError if the shape could not be retrieved, otherwise a vector of the shape.
     */
    Result<std::vector<DimensionIndex>> get_store_shape() const {
        ...
        if (!json["metadata"].contains("shape")) {
            return absl::NotFoundError("Metadata['attributes'] did not contain key 'shape'.");
        }
     ...
     * This method should not be called independantly as it will result in a mismatch between the Variable metadata and Dataset metadata
     * @return 
     */
    Future<tensorstore::TimestampedStorageGeneration> PublishMetadata() {
        auto publish = [](const ::nlohmann::json& json_var, bool isCloudStore, const tensorstore::TensorStore<T, R, M>& store) -> Future<tensorstore::TimestampedStorageGeneration> {
            auto output_json = json_var;
            
            output_json["attributes"]["_ARRAY_DIMENSIONS"] = output_json["dimension_names"];
            output_json.erase("dimension_names");
            if (output_json.contains("long_name")) {
            ...
            return tensorstore::kvstore::Write(store.kvstore(), outpath, absl::Cord(output_json["attributes"].dump()));
        };
        ...
        auto isCloudStoreRF = tensorstore::MakeReadyFuture<bool>(isCloudStore);
        auto metadataRF = tensorstore::MakeReadyFuture<::nlohmann::json>(this->getMetadata());
        auto storeRF = tensorstore::MakeReadyFuture<tensorstore::TensorStore<T, R, M>>(store);
        auto write_metadata_future =
        tensorstore::MapFutureValue(tensorstore::InlineExecutor{}, publish, metadataRF, isCloudStoreRF, storeRF);
        auto pair = tensorstore::PromiseFuturePair<tensorstore::TimestampedStorageGeneration>::Make();
        write_metadata_future.ExecuteWhenReady([this, promise = std::move(pair.promise)](tensorstore::ReadyFuture<tensorstore::TimestampedStorageGeneration> readyFut) {
            auto ready_result = readyFut.result();
            if (!ready_result.ok()) {
                promise.SetResult(ready_result.status());
            } else {
    ...
    /**
     * @brief Attempts to safely update the user attributes of a Variable (["metadata"]["attributes"] and/or ["metadata"]["statsV1"]).
     * @tparam T_attrs The optional type of the histogram. Must be either int32_t or float (Default: float)
     * @param newAttrs The JSON representation of a UserAttribute.
     * @return An OK status if the attributes were successfully updated, otherwise an error and the attributes remain unchanged.
     * @details \b Intended_Usage
     * @code
     * auto var = my_dataset.variables.at("my_variable_key").value(); // Status check ignored for berevity
     ...
     * // NOTE: The vector is of type int32_t. We should pass the type as a template argument.
     * auto update_result = var.UpdateAttributes<int32_t>(to_update);
     * if (!update_result.status().ok()) {
     *      // In this case nothing will happen. We output an error and move on with what previously existed.
     *      std::cerr << "Failed to update attributes: " << update_result.status() << std::endl;
     * }
     * @endcode
     ...
     * NOTE: This may only be useful for internal uses. See `getMetadata()` for the full metadata.
     * This version should lack the mutable UserAttributes portions, if they exist
    ...
    */
    const bool was_updated() const {
        ...
        // This works because the UserAttributes object is immutable and can only be replaced.
        std::uintptr_t currentAddress = reinterpret_cast<std::uintptr_t>(&(**attributes));
    ...
    const tensorstore::TensorStore<T, R, M>& get_store() const {
        return store;
    }
...
private:
     ...
     * This method is intended to be called as a callback by the Dataset CommitMetadata method after the updated data
     * is committed to durable media.
     * @brief Updates the current address of the User Attributes.
     */
    void _dataset_only_callback_committed() {
        // We only want to update the address if the UserAttributes object has changed location
        // This indicates a new UserAttributes object has taken the place of the existing one.
        if (attributes.get() != nullptr && attributes->get() != nullptr) {
            std::uintptr_t newAddress = reinterpret_cast<std::uintptr_t>(&(**attributes));
            // TODO: Leaving this as active is causing segfaults. The features requiring it are low priority.
            // attributesAddress = newAddress;
        }
        // It is fine that this will only change in the "collection" instance of the Variable, because
        // that is the only one that will be operated on by the Dataset commit
    }
...
// Tensorstore Array's don't have an IndexDomain and so they can't be slice with labels e.g. "inline", "crossline".
/**
 * @brief A LabeledArray is an underlying data type for a VariableData object and is not intended to be directly
 * interacted with.
 */
template <typename T, DimensionIndex R, ArrayOriginKind OriginKind>
struct LabeledArray {
    ...
    LabeledArray(const tensorstore::IndexDomain<R>& dom, const shared_array& arr) : domain(dom), data(arr){};

    /**
     * @brief Slices the tensor along the specified dimensions and returns the resulting sub-tensor.
     * @pre The step of the slice descriptor must be 1.
     * @pre The start and stop of the slice descriptor must be in the domain of the tensor.
     * @tparam alloc The allocation constraint to use.
     * @tparam Descriptors The types of the descriptors used to specify the slice.
     * @param descriptors The descriptors used to specify the slice.
     * @return An `mdio::Result` containing a `const_shared_array` representing the sliced tensor on success, or
    ...
    Result<const_shared_array> slice(Descriptors&... descriptors) {
        ...
        std::vector<Index> start(numDescriptors), stop(numDescriptors), step(numDescriptors);
        ...
        std::apply(
            [&](const auto&... desc) {
                ...
                ((
                     ...
                     start[idx] = desc.start,
                     stop[idx] = desc.stop,
                     step[idx] = desc.step,
                     // Resolve the label and capture any error
                     [&]() {
                         auto result = tensorstore::Dims({desc.label}).Resolve(domain, &buffer);
                         if (!result.ok()) {
        ...
        return data | tensorstore::Dims(dims).TranslateHalfOpenInterval(start, stop, step) |
               tensorstore::Materialize(alloc);
    };

    SharedArray<T, R, OriginKind> get_data() {
        return data;
    }
 ...
 * This data only exists in memory and should be preserved with the Variable::Write() function.
 */
template <typename T, DimensionIndex R, ArrayOriginKind OriginKind>
struct VariableData {
    VariableData(const std::string& variableName,
                 const std::string& longName,
                 const ::nlohmann::json& metdata,
                 const LabeledArray<T, R, OriginKind>& data)
        : variableName(variableName), longName(longName), metadata(metdata), data(data){};

    // Allows for conversion to compatible types (SourceElement), which should always be possible to void.
    template <typename SourceElement,
              DimensionIndex SourceRank,
              ArrayOriginKind SourceOriginKind>
    VariableData(const VariableData<SourceElement, SourceRank, SourceOriginKind>& other)
        : variableName(other.variableName), longName(other.longName), metadata(other.metadata), data(other.data){};
     ...
     * @return mdio::Result<LabeledArray<T, R, OriginKind>> The sliced LabeledArray.
     */
    template <MustAllocateConstraint alloc = MustAllocateConstraint::may_allocate,
              typename... Descriptors>
    Result<const_shared_array> slice(Descriptors&... descriptors) {
     ...
     * This is the in-memory representation of the data. You should be interacting with this object
     * instead of the data held in Variable.
     * @return mdio::SharedArray<T, R, OriginKind, tensorstore::container> The underlying SharedArray that can be
     * manipulated as desired.
     */
    SharedArray<T, R, OriginKind> get_data_accessor() {
        return data.get_data();
    }

    /**
     * @brief Calculates the offset of the flattened data.
     * This is useful when using `get_data_accessor().data()` because the 0'th element may not be the start of the data and
     ...
     * MDIO_ASSIGN_OR_RETURN(auto sliced_variable, sliced_dataset.variables.get<mdio::dtypes::float32>("my_variable"));
     * MDIO_ASSIGN_OR_RETURN(auto sliced_data, sliced_variable.Read().result());
     * auto flattened_data = reinterpret_cast<mdio::float32_t*>(sliced_data.get_data_accessor().data());
     * auto offset = sliced_data.get_flattened_offset();
     * // We can use the method `sliced_data.get_data_accessor().num_elements()` in conjunction with 
    ...
    ptrdiff_t get_flattened_offset() {
        ...
        auto origin_ptr = accessor.data(); // The raw pointer to the data. May not start at 0.
        auto offset_ptr = accessor.byte_strided_origin_pointer(); // The raw pointer to the first element of the data.
        char* origin_addr = reinterpret_cast<char*>(origin_ptr);
        char* offset_addr = reinterpret_cast<char*>(offset_ptr.get()); // We have to get the raw pointer
...
/**
 * @brief If you already have a variable in memory, allocated a variableDataobject using this variable as the
 * specification.
 */
template <typename T = void,
          DimensionIndex R = dynamic_rank,
...
Result<VariableData<T, R, OriginKind>> from_variable(const Variable<>& variable) {
    ...
    // There are two steps here, first create a variable with the compile time data type as "void", and the internal
    // data type of the array according to the variable
    auto _array = tensorstore::AllocateArray(variable.get_store().domain().box(),
                                             mdio::ContiguousLayoutOrder::c,
                                             tensorstore::value_init,
                                             variable.dtype());

    // The second step tries to cast the dtype of the array to the supplied templated.
    // this can fail if the types are inconsistent, at which point it will return with a status.
    MDIO_ASSIGN_OR_RETURN(auto array, tensorstore::StaticDataTypeCast<T>(std::move(_array)))

    auto labeled_array = LabeledArray<T, R, OriginKind>{domain, std::move(array)};

    return VariableData<T, R, OriginKind>{
        variable.get_variable_name(), variable.get_long_name(), variable.getReducedMetadata(), std::move(labeled_array)};
}
}; // namespace mdio
#endif
