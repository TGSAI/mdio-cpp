...
namespace mdio {
    namespace utils {
         ...
         * DANGER: This operation will mutate the dataset on disk. Use caution when calling this method!
         * This function is not part of the Dataset class to avoid accidental data destruction.
         * Additionally this function should only be used on a fully written dataset to avoid race conditions and data corruption.
         * This may make writing generalized functions more difficult, but we elected to err on the side of caution.
         * 
         * @tparam ...Descriptors Expects an mdio::SliceDescriptor
         * @param dataset_path The path to the dataset to trim.
         * @param descriptors The descriptors to use for the slice. Only considers the label and stop value.
         * @return A future of the trim operation.
        */
        template <typename... Descriptors>
        Future<void> TrimDataset(std::string dataset_path, const Descriptors&... descriptors) {
            // Open the dataset
            ...
            for (auto& varIdentifier : ds.variables.get_iterable_accessor()) {
                ...
                if (var.dimensions().labels().back() == "") {
                    ...
                    nlohmann::json specJson = specJsonResult.value()["metadata"]["dtype"][0][0];
                    std::string field = specJson.get<std::string>();
                    // If the variable is structured data we will pick the first dimension arbitrarially
                ...
                auto resizedStatus = tensorstore::Resize(
                    varStore,
                    tensorstore::span<const tensorstore::Index>(implicitDims),
                    tensorstore::span<const tensorstore::Index>(newShape),
                    tensorstore::ResizeOptions{tensorstore::ResizeMode::resize_tied_bounds});
         ...
         * It will first be checked that the dataset is a valid MDIO dataset before deletion.
         * This is intended to provide a safe interface to delete MDIO datasets.
         * @param dataset_path The path to the dataset
         * @return OK result if the dataset was valid and deleted successfully, otherwise an error result
         */
        Result<void> DeleteDataset(std::string dataset_path) {
            // Open the dataset
            // This is to ensure that what is getting deleted by MDIO is a valid MDIO dataset itself.
            auto dsRes = mdio::Dataset::Open(dataset_path, mdio::constants::kOpen);
            if (!dsRes.status().ok()) {
                return dsRes.status();
            }
            ...
            try {
                ...
                return absl::InternalError("Failed to delete dataset: " + std::string(e.what()));
            }
